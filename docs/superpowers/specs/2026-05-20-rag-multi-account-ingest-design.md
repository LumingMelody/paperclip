...
> **修正（2026-05-20，执行期实查）**：`dm_allretrun_analysis_d.Account` 实际取值是 **`Amazon{品牌}{国家}`** 风格（`AmazonEPUS`、`AmazonEPUK`、`AmazonEPDE` …），**不是** `EverPretty-*`。早先「EverPretty-US 风格」的说法（钉钉文档 §2.1、RAG memory）有误；`client.ts::shopToAccount` 的 `AmazonEP{XX}` 才对。DB `SELECT DISTINCT Account` 为权威。

EP 的 Amazon 站点共 8 个：`AmazonEP` + `CA/DE/ES/FR/IT/MX/UK/US`。（`EPSITE*` 是 EP 独立站渠道，不在 B1 的 `EP-XX↔AmazonEPXX` shop 模型内，本期不含；`AmazonPZ*`/`AmazonDAMA*` 是其他品牌，本期不含。）
...
account → shop 逆映射：`AmazonEPUK → EP-UK`（正则 `^Amazon(EP|PZ|DAMA)([A-Z]{2})$` → `{品牌}-{国家}`，是 `shopToAccount` 的逆）。
