You are Codex. Write a **one-off Python analysis script** that pulls two specific
SKU prefixes' full-view metrics across Amazon (DWS prod) + Shopify (OMS) and
emits a markdown comparison report.

This is NOT production code — it's a data deliverable for an ad-hoc question.
Don't worry about reusability or generic CLI design. Just hard-code the two
SKU prefixes and emit one report.

## Why DWS for Amazon

Earlier attempt used OMS `amazon_order_item` for Amazon sales, but OMS dev DB
is sparse (~1k units for 2088). DWS prod table `dws_od_amazon_refund_rate_d`
has the real numbers (~53k units sold + ~34k refunded for 2088). Use DWS for
both numerator AND denominator on Amazon = consistent return rate.

Shopify data IS complete in OMS dev (1,665 units for 2088). Use OMS for
Shopify side.
...
| Source | Conn | Use for |
|---|---|---|
| DWS Aliyun RDS | host=rm-bp1dm282ayh5203tngo.mysql.rds.aliyuncs.com:3306, user=DW_AI_READ_ONLY, password=epai@123456, db=everpretty | Amazon sales + returns (truth) |
| OMS MySQL | host=192.168.0.227:3306, user=root, password=@@OMSWMStms2025##, db=dev_everpretty_oms | Shopify orders, inventory, sku_base |

Tables:
- DWS `dws_od_amazon_refund_rate_d` cols: `seller_sku`, `quantity` (=sold), `rf_quantity` (=refunded), `check_date`, `amazon_order_id`, `seller_color`, `seller_style`
- DWS `dm_allretrun_analysis_d` cols: `sku`, `return_reason`, `quantity`, `orderid`, `date`, `Account` (for reason distribution)
- OMS `shopify_order` (id PK, `shopify_order_id` is **string** external id, `currency`, `customer_email`, `order_created_at`, `cancelled_at`, `financial_status`)
- OMS `shopify_order_item` (`sku`, `quantity`, `price`, `shopify_order_id` is **bigint** external id — JOIN via `CAST(o.shopify_order_id AS UNSIGNED) = i.shopify_order_id`)
- OMS `inventory` (`sku_code`, `warehouse_id`, `available_quantity`, `physical_quantity`, `transit_in_quantity`, `deleted`)
- OMS `warehouses` (`id`, `warehouse_code`, `warehouse_name`, `warehouse_type`, `country_code`, `is_active`)
- OMS `sku_base` (`sku`, `is_delete`, `product_name`; NO created_at/updated_at)

## Two SKUs to analyze

- **2088 系列**: prefix `EG02084`
- **2559 系列**: prefix `EE02559`

## Time window

All-time (no date filter on sales side). For repurchase + Shopify, just use everything in the dev DB.

## Shell rules

**Allowed**: `cat`, `ls`, `python3 docs/sku-analysis/<script> ...` to execute.
**Forbidden**: `git`, `pnpm`, `pip install` (use only pymysql which is already installed).

## What to do

### File 1 (create): `docs/sku-analysis/_analyze_sku_full_view.py`

A standalone Python script (≈250 lines). Structure:
...
- Verbatim copy of the file above. No other files touched.
- Run it once and verify output exists at `docs/sku-analysis/<date>-EG02084-vs-EE02559.md`.

## Report

1. `wc -l docs/sku-analysis/_analyze_sku_full_view.py`
2. `python3 docs/sku-analysis/_analyze_sku_full_view.py` — should print `Wrote ...`
3. `wc -l docs/sku-analysis/2026-05-13-EG02084-vs-EE02559.md`
4. Show first 5 lines of the comparison-table section ("## 两款对比一表" + 4 following lines)
