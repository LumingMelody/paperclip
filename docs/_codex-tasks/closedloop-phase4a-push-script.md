You are Codex implementing one bash helper script: `paperclip-dingtalk-push.sh`.
Mirrors `scripts/paperclip-issue-update.sh` in style — small, single-purpose,
jq-built payload, dry-run support.

## Purpose

Push a markdown card to a DingTalk **custom group robot webhook**. Used by
the 5 routine agents after they write a weekly/monthly report, so Anna sees
the report (and its embedded `S1/S2/S3` suggestions) on her phone without
having to open the paperclip Web UI.

## DingTalk webhook spec (security: HMAC-SHA256)

DingTalk custom robot webhooks accept signed POSTs. The signing scheme:

```
timestamp = current millis (string)
string_to_sign = f"{timestamp}\n{secret}"
sign = url_encode(base64(hmac_sha256(secret, string_to_sign)))
url = f"{webhook_url}&timestamp={timestamp}&sign={sign}"
```

Then POST JSON body to that signed URL:
...
(Curl + openssl + jq + python3 -c is fine. No need for a Python venv.)

## Env vars

| Var | Required | Purpose |
|---|---|---|
| `DINGTALK_WEBHOOK_URL` | yes (POST mode) | Custom robot webhook URL (no `&timestamp/sign`) |
| `DINGTALK_WEBHOOK_SECRET` | yes (POST mode) | HMAC secret from the robot setup screen |

(Both are read from env. `--dry-run` doesn't require them — emits the would-be URL + payload to stdout.)

## Shell rules

**Allowed**: `cat`, `ls`, `rg`, `sed -n`.
**Forbidden**: `git`, `chmod`, `pnpm`, network. Claude commits and chmods.

## What to do

### File 1 (create): `scripts/paperclip-dingtalk-push.sh`
...
- Verbatim copy. No extra comments.
- Don't `chmod`. Claude does that after commit.
- Don't touch other files.

## Report

1. `wc -l scripts/paperclip-dingtalk-push.sh`
2. `bash -n scripts/paperclip-dingtalk-push.sh`
3. Exercise `--dry-run` with sample args; confirm payload prints.
