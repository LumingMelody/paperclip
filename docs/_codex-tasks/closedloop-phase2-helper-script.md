You are Codex implementing one bash helper script for **Phase 2 of the
closed-loop suggestion tracking system**: a small CLI that posts a structured
suggestion to the paperclip API. Pattern mirrors `scripts/paperclip-issue-update.sh`
exactly.

## Pattern reference

`scripts/paperclip-issue-update.sh` (already in repo) is the canonical helper
for paperclip API writes from inside an agent run. Read it for style. The new
script for suggestions follows the same shape:
- env: `PAPERCLIP_API_URL`, `PAPERCLIP_API_KEY`, `PAPERCLIP_RUN_ID`,
  `PAPERCLIP_COMPANY_ID`, `PAPERCLIP_AGENT_ID`, `PAPERCLIP_TASK_ID`
- supports --dry-run
- jq-builds JSON payload (preserves any nested objects)
- helpful error messages on missing args

## Shell rules

**Allowed**: `cat`, `ls`, `rg`, `sed -n`.
**Forbidden**: `git`, `chmod` outside the new file, `pnpm`, network, anything else. Claude commits + makes executable.

## What to do

### File 1 (create): `scripts/paperclip-suggestion-create.sh`
...
- Copy verbatim. No extra comments.
- Don't `chmod +x` the script — Claude does that after commit.
- Don't touch any other file.

## Report

1. `wc -l scripts/paperclip-suggestion-create.sh`
2. `head -1 scripts/paperclip-suggestion-create.sh` (verify shebang)
3. `bash -n scripts/paperclip-suggestion-create.sh` (syntax check)
4. Optionally exercise --dry-run with sample args to verify jq payload structure (do NOT exercise without --dry-run since that POSTs to the real API)
5. Any deviations (should be none).
