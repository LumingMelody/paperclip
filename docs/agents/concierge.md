...
| 「EP-US Top 20 畅销款里退货率最高的几款」 | 先 `lingxing.topSkus` 拿畅销榜（GMV 口径），再对榜里每个款用 `dws.returnRateByStyle(style=...)` 取率 |
...
口径（工具内已固化）：GMV=不含礼品卡的实收金额，销量=排除 YS 保险件，订单数=去重 order_id；已滤掉无 SKU 的财务补充行；时间维度 `statistic_time_local`。输入 `since`，可选 `until`(exclusive) / `groupBy`(platform|account|bu|country|day|month|none，默认 platform) / `platform` 过滤 / `top`：
...
⚠️ 这是**跨平台汇总口径，不做单 SKU 维度**。要 **Amazon 单 SKU/ASIN 的 GMV 明细 / 畅销榜** 仍走 `lingxing.topSkus` / `lingxing.styleSummary` / `lingxing.factSku`；要 **Amazon 单款新鲜件数** 走 `dws.amazonSalesByStyle`；要 **独立站单款件数** 走 `dws.siteTopStyles`。`oms.salesByChannel` 仅在需要 OMS 内部渠道视角时用，公司级总额以 `dws.salesSummary` 为准。
...
⚠️ **只有件数（salesQty/orderCount/skuCount），没有 GMV** —— 要**公司级 / 按平台·店铺 的 GMV** 走 `dws.salesSummary`；要 **Amazon 单 SKU/ASIN 的 GMV 明细**、评分、评论、广告数据，仍走 `lingxing.styleSummary` / `lingxing.factSku` / `lingxing.topSkus`。
